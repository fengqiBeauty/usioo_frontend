...
<div v-if="eval">
  ...
  <textarea class="remark pj" placeholder="说说您的评价" v-model="remark"></textarea>
	<p class="title">其他建议</p>
	<textarea class="remark pj" placeholder="说说您的建议" v-model="suggest"></textarea>
...
import { mental_evaluate,weixin_login } from 'api/common'
export default {
  name: 'endorder',
  data () {
    return {
  ...
  methods: {
    ...
    clickFn: function () {
      mental_evaluate({
					mental_record_id: this.mental_record_id,
					content: this.remark,
					star: this.fy,
  ...
  created: function () {
    this.mental_record_id = window.location.href.split('#pid')[1];
    console.log(this.mental_record_id)
  },
  mounted () {
