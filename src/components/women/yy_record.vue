...
<div class='record'>
  ...
  <mt-tab-container v-model="selected" v-for='(item, index) in list' :key='item.id'>
    <div class="card" >
      <yd-accordion>
        <yd-accordion-item :title="item.mental_title">
  ...
  <div class="f-wu" v-if="!list.length">暂时没有内容~</div>
...
import { mental_record,weixin_login } from 'api/common'
export default {
  name: 'List',
  data () {
    return {
  ...
  methods: {
    toeva: function (a) {
      if (this.selected === a.status) {
        this.$router.push({path: '/women/yy_eval#pid' + a.id});
      }
    },
    // 获取各个状态的评价
