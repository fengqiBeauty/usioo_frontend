    ...
    <div v-if="this.content">
      ...
      <!-- swiper -->
        <swiper :options="swiperOption" ref="mySwiper">
          <swiper-slide v-for="(item,index) in act_list" :key='index' @click.native='to_details'>
...
export default {
  ...
  methods: {
    to_details(){
      ...
      this.$router.push({path:'/community/act_detail'});
    }
  },
  mounted () {
    sq_community({
...
<style scoped>
.hd_title{font-weight: 700;width: 100%;text-align: center;margin-bottom: 20px;}
.swiper-inner {
  width: 100%;
  margin: 0 auto;
  padding-top: 20px;
...
.s_rate{display: -webkit-box;display: -webkit-flex;display: flex;align-items: center;}
...
.look_del{background: linear-gradient(to right, #4081CD,#4081CD,#4081CD,#76BAFB);
	box-shadow: none;}
